...
const ResumeCard = ({ resume }: { resume: Resume }) => {
  ...
  useEffect(() => {
    // Only generate previews when the card is close to viewport.
  ...
  useEffect(() => {
    ...
    // Fast cache: sessionStorage (per tab) to avoid re-generating when navigating back.
    try {
    ...
    const schedule = (fn: () => void) => {
      // Prefer idle time so we don't block scrolling.
    ...
    schedule(() => {
      // Allow one paint where the hidden preview exists before snapshot.
      setTimeout(() => {
        void run();
      }, 0);
    });
        ...
        {shouldGeneratePreview && !previewRendered && (
          <div className="fixed -left-[9999px] -top-[9999px]">
            ...
            >
              ...
              >
                <template.template
                  data={resume.content as unknown as T_Resume}
                />
              </div>
            </div>
          </div>
