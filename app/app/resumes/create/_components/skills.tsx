    ...
    >
      <div className="bg-background p-4 space-y-5">
        <div className="flex flex-col gap-0.5">
          <span className="font-semibold font-serif">Skills</span>
          <span className="text-xs font-light text-foreground/50">
            Choose 5 importants skills that show you fit the position. Make sure
            they match the key skills mentioned in the job listening.
          </span>
        </div>
        <SortableContext
          items={data.skills.map((item) => item.id)}
          strategy={verticalListSortingStrategy}
        >
          {data.skills.map((item) => (
            <SkillCard key={item.id} data={item} setData={setData} />
          ))}
        </SortableContext>
