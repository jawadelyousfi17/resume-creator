...
import { reactions } from "../professionalSummary";
import { enhanceJobDescription } from "@/actions/openai/enhaceJobDescription";
...
const EmployerCard = ({
  ...
  data: T_Resume["employmentHistory"][0];
  ...
  async function handleDelete(toDeletId: string) {
    ...
    if (confirmed) {
      setData((p) => ({
        ...p,
        employmentHistory: p.employmentHistory.filter(
          (item) => item.id !== toDeletId
        ),
      }));
    }
  }
  ...
  async function handleEnhaceJobDescription() {
    try {
      ...
      const confirmed = await confirm({
        title: "This is the suggested description",
                  ...
                  <div className="flex flex-col gap-1 flex-1 ">
                    ...
                    <div className="h-auto  w-full bg-muted rounded-lg flex flex-col  overflow-hidden">
                      <HtmlEditor
                        sufix={
                          <Button
                            onClick={handleEnhaceJobDescription}
                            className="bg-background  rounded-full text-indigo-500 hover:text-indigo-700 hover:bg-background/80 float-right"
                            size="sm"
                            disabled={loading}
                          >
                            {loading && <Spinner />}
                        ...
                        onChange={(html) =>
                          setData((prev) => ({
                            ...prev,
                            employmentHistory: prev.employmentHistory.map(
                              (item) => {
                                if (item.id === data.id)
                                  return {
                                    ...item,
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-light text-foreground/50">
                        Tip: write 400+ characters to increase interview chances
                      </span>
