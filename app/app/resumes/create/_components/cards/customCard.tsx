...
const CustomCard = ({
  ...
  data: T_Resume["additional"][0]["sections"][0];
  ...
  const [enhacing, setEnhacing] = useState(false);

  async function handleDelete(toDeletId: string) {
    const confirmed = await confirm({
      title: "Delete Item?",
      description: "This will permanently remove this item from your resume.",
      confirmText: "Delete",
      cancelText: "Cancel",
      variant: "destructive",
    });
      ...
      setData((prev) => ({
        ...
        additional: prev.additional.map((collection) => {
          if (collection.id === collectionId) {
            return {
              ...collection,
              sections: collection.sections.filter(
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              <div className="text-sm font-semibold group-hover:text-primary">
                {data.activity ? data.activity : "Untitled"}
              </div>
                      ...
                      additional: prev.additional.map((collection) => {
                        if (collection.id === collectionId) {
                          return {
                            ...
                            sections: collection.sections.map((item) => {
                              if (item.id === data.id)
                                return { ...item, activity: e.target.value };
                              return item;
                            }),
                          };
                        }
                      ...
                      <HtmlEditor
                        sufix={
                          ...
                          >
                            ...
                            {enhacing ? "Enhancing..." : "Enhace with AI"}
                          </Button>
                        }
