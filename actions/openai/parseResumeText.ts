...
export type ParseResumeTextResult = {
  data?: T_Resume;
  error?: string;
  raw?: string;
};
...
export async function parseResumeTextToResume(
  resumeText: string
): Promise<ParseResumeTextResult> {
  if (!resumeText?.trim()) {
...
- Ensure every list item has an "id" field (can be a short unique string).
